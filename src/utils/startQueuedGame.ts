...
export const startQueuedGame = async (deploymentTime: number) => {
    ...
    for (const group of groups) {
        ...
        const playerEmbed = buildEmbed({ preset: "success" })
            ...
            .setDescription(
                `${ client.battalionStrikeMode ? `You have been assigned to ${hostDisplayName}'s strike group!` : 'You have been selected for a HOTDROP deployment!' }\n\n` +
                `**Code:** ${randomCode}\n` +
        ...
        const hostEmbed = buildEmbed({ preset: "success" })
            ...
            .setDescription(
                `${ client.battalionStrikeMode ? `Your strike group has been assembled!` : 'Your Hotdrop deployment squad has been assembled!' }\n\n` +
                `**Code:** ${randomCode}\n` +
                `**Voice Channel:** <#${vc.id}>\n\n` +
