    ...
    func: async function ({ interaction }) {
        try {
            ...
            if (!joinTime) {
                ...
                const errorEmbed = buildEmbed({ preset: "error" })
                    .setDescription("Could not find when you joined the queue!\nPlease submit a bug report using `/bugeport`");
                    ...
                    await message.edit({
                        embeds: [
                            ...
                            buildEmbed({ preset: "error" })
                                .setColor('#FF0000')
                                .setTitle(`You've left the ${client.battalionStrikeMode ? 'strike' : 'hotdrop'} queue!`)
                                .setDescription(
