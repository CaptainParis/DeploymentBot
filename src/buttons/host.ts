...
export default new Button({
    ...
    func: async function({ interaction }) {
        ...
        if (hostsInQueue.length >= config.queueMaxes.hosts && !client.battalionStrikeMode) {
            const errorEmbed = buildEmbed({ preset: "error" })
                .setDescription(`The ${client.battalionStrikeMode ? 'strike' : 'hotdrop'} queue is currently full!`);
                ...
                if (existingMessage) {
                    receiptMessage = await existingMessage.edit({
                        embeds: [buildEmbed({ preset: "success" })
                            ...
                            .setTitle(`You've joined the ${client.battalionStrikeMode ? 'strike' : 'hotdrop'} queue as a host!`)
                            .setDescription(
